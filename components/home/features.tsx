...
const CARD_TRANSITION = { duration: 0.5, delay: 0.5 } as const;
const EARTH_MARKER: [number, number, number] = [0, 0, 0];
const EARTH_GLOW: [number, number, number] = [1, 0.3, 0.4];
const INVIEW_OPTS = { once: true, amount: 0.3 } as const;

interface CardWithRefProps {
  isInView: boolean;
...
const PixelCard = memo(function PixelCard({
  ...
  isLight,
  cardRef,
}: CardWithRefProps & { isLight: boolean }) {
    ...
    >
      <div className="z-10 flex flex-col gap-4">
        ...
        <div className="text-md text-muted-foreground flex flex-col gap-2 text-sm">
          <p>
            Every block comes perfectly styled and optimized for all screen
            sizes, so your UI looks great out of the box.
          </p>
        </div>
      </div>
...
const CliCard = memo(function CliCard({
  isInView,
  isDark,
  cardRef,
}: CardWithRefProps & { isDark: boolean }) {
  const animate = isInView ? MOTION_VISIBLE : MOTION_HIDDEN;
  return (
    <motion.div
      ref={cardRef}
...
interface GlobeCardProps extends CardWithRefProps {
  baseColor: [number, number, number];
  dark: number;
}

const GlobeCard = memo(function GlobeCard({
  isInView,
  baseColor,
  dark,
  cardRef,
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        ...
        <div className="text-md text-muted-foreground flex flex-col gap-2 text-sm">
          <p className="max-w-[460px]">
            Blocks are available everywhere but ours are the best. Use them in
            your favorite framework or even in plain HTML.
          </p>
        </div>
      </div>
      <div className="flex min-h-[300px] grow items-start justify-center select-none">
        <h1 className="mt-8 text-center text-5xl leading-[100%] font-semibold sm:leading-normal lg:mt-12 lg:text-6xl">
          <span className='bg-background relative mt-3 inline-block w-fit rounded-md border px-1.5 py-0.5 before:absolute before:top-0 before:left-0 before:z-10 before:h-full before:w-full before:bg-[url("/noise.gif")] before:opacity-[0.09] before:content-[""]'>
            <ScrambleHover
              ...
              setIsHovering={setIsHoveringHandler}
        ...
        <div className="absolute top-48 bottom-0 z-10">
          ...
          >
            <Earth
              baseColor={baseColor}
              markerColor={EARTH_MARKER}
              glowColor={EARTH_GLOW}
              dark={dark}
            />
          </Suspense>
        </div>
...
export default function Features() {
  ...
  const headerInView = useInView(headerRef, INVIEW_OPTS);
  const pixelInView = useInView(pixelRef, INVIEW_OPTS);
  const cliInView = useInView(cliRef, INVIEW_OPTS);
  const globeInView = useInView(globeRef, INVIEW_OPTS);
  const { resolvedTheme } = useTheme();

  const isDark = resolvedTheme === 'dark';
  const isLight = resolvedTheme === 'light';

  const { baseColor, dark } = useMemo<{
    baseColor: [number, number, number];
    dark: number;
  }>(
    () =>
  ...
  const headerAnimate = headerInView ? MOTION_VISIBLE : MOTION_HIDDEN;
